...
DirtyLookAndFeel1::DirtyLookAndFeel1()
{
    ...
    image_knob_strip_png = ImageCache::getFromMemory(pngBinaries::knob_strip_png, pngBinaries::knob_strip_pngSize);
}
...
void DirtyLookAndFeel1::drawRotarySlider(Graphics& g, int x, int y, int width, int height, float sliderPos, const float rotaryStartAngle, const float rotaryEndAngle, Slider& slider)
{
    ...
    if(image_knob_strip_png.isValid())
    {
        ...
        const float knob1_width = image_knob_strip_png.getWidth();
        const float knob1_height = image_knob_strip_png.getHeight();
        ...
        g.drawImage(image_knob_strip_png, (int)rx, (int)ry, 2 * (int)radius, 2 * (int)radius, 0, frameId * knob1_width, knob1_width, knob1_width);
